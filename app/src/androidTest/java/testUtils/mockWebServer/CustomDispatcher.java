...
public class CustomDispatcher extends Dispatcher {
    ...
    public static boolean compareLocationJsonObjects(JSONObject actualRequestBodyOfSyncRequest, JSONObject expectedRequestBodyOfSyncRequest) throws JSONException {
        ...
        Iterator actualRequestJsonIterator = actualRequestBodyOfSyncRequest.keys();
        Iterator expectedRequestJsonIterator = expectedRequestBodyOfSyncRequest.keys();

        if (actualRequestBodyOfSyncRequest != null && expectedRequestBodyOfSyncRequest != null) {
            while (expectedRequestJsonIterator.hasNext()) {

                String expectedJsonCurrentKey = (String) expectedRequestJsonIterator.next();
                while (actualRequestJsonIterator.hasNext()) {

                    String actualJsonCurrentKey = (String) actualRequestJsonIterator.next();

                    if (expectedJsonCurrentKey.equals("data") && expectedJsonCurrentKey.equals(actualJsonCurrentKey)) {
        ...
        LogUITest.debug("Request Body is not found either in Actual Request Or in Expected Request Body");
        LogUITest.debug("Request Body in Actual Request : " + actualRequestBodyOfSyncRequest);
        LogUITest.debug("Request Body in Expected Request : " + expectedRequestBodyOfSyncRequest);
        return false;
